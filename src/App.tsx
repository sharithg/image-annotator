    ...
    return (
        <div>
            ...
            <div className="control-container">
                <button
                    onClick={() =>
                        setRotation((rotation + 90) as DegreeRotations)
                    }
                >
                    Rotate
                </button>
                ...
                <ul>
                    {boundingBoxAnnotations.map((annotation) => (
                        <li
                            ...
                            key={annotation.id}
                        >
                            X: {annotation.x.toFixed(2)}, Y:{' '}
                            {annotation.y.toFixed(2)}, H:{' '}
                            {annotation.height.toFixed(2)}, W:{' '}
                            {annotation.width.toFixed(2)}
                        </li>
                    ))}
                </ul>
                ...
                <ul>
                    {lineAnnotations.map((annotation) => (
                        <li
                            ...
                            key={annotation.id}
                        >
                            X0: {annotation.x1.toFixed(2)}, Y0:{' '}
                            {annotation.y1.toFixed(2)}, X1:{' '}
                            {annotation.x2.toFixed(2)}, Y1:{' '}
                            {annotation.y1.toFixed(2)}
                        </li>
                    ))}
                </ul>
            </div>
